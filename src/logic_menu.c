...
static void PrintLogicMenuHints();
...
void CB2_InitEvidenceMenu(void)
{
    FadeScreen(FADE_TO_BLACK, 0);
    CreateTask(Task_OpenEvidenceMenu, 1);
}

void Task_OpenEvidenceMenu(u8 taskId)
{
...
static void Task_LogicMenuHandleDeduction(u8 taskId)
{
    ...
    case 0: {
        ...
        PlaySE(SE_SHINY);
        ForceHideScrollArrows(sLogicMenuState->scrollIndicatorTask, TRUE);
    ...
    case 1:
        if (JOY_NEW(A_BUTTON))
        {
            ...
            ForceHideScrollArrows(sLogicMenuState->scrollIndicatorTask, FALSE);
            ...
            if (list->template.totalItems < list->template.maxShowed)
                list->template.maxShowed = GetListMaxShowable(&list->template);
            else
...
static u32 CreateEvidenceListMenu(struct ListMenuTemplate* template)
{
    ...
    *t = sLogicMenuListTemplate;
    ...
    gTempScrollArrowTemplate.fullyDownThreshold = t->totalItems - t->maxShowed;

    sLogicMenuState->scrollIndicatorTask = AddScrollIndicatorArrowPair(
...
static void DestroyEvidenceIcon(u32 pos)
{
    ...
    FreeSpriteOamMatrix(&gSprites[sLogicMenuState->spriteIds[pos]]);
...
static void LogicMenuList_PrintFunc(const struct ListMenu *list, u32 index, u8 y)
{
    static const u8 sBaseColors[] = {
        TEXT_COLOR_TRANSPARENT,
        TEXT_COLOR_DARK_GRAY,
        TEXT_COLOR_LIGHT_GRAY,
    };

    static const u8 sSelectedColors[] = {
        TEXT_COLOR_TRANSPARENT,
        TEXT_COLOR_BLUE,
        TEXT_COLOR_LIGHT_GRAY,
    };
    ...
    bool32 selected =
        lsearch(&id, sLogicMenuState->selected, sLogicMenuState->maxSelections) !=
        UINT32_MAX;

    const u8 *colors = selected ? sSelectedColors : sBaseColors;
    ...
    AddTextPrinterParameterized4(windowId, fontId, x, y, 0, 0, colors, 0, name);
...
static bool8 LogicMenu_LoadGraphics(void)
{
    ...
    case 1:
        ...
        break;
    case 2:
        LoadBgTiles(2, GetWindowFrameTilesPal(gSaveBlock2Ptr->optionsWindowFrameType)->tiles, 0x120, MAIN_MENU_BORDER_TILE);
...
static void DrawLogicMenuWindowBorder(const struct WindowTemplate *template, u16 baseTileNum)
{
...
static void LogicMenu_InitWindows(void)
{
    ...
    LoadMessageBoxGfx(WIN_LOGIC_MSG, BIRCH_DLG_BASE_TILE_NUM, BG_PLTT_ID(15));
...
static void DrawLogicMenuMsgWin(u8 winId)
{
    ...
    DrawDialogFrameWithCustomTile(winId, TRUE, BIRCH_DLG_BASE_TILE_NUM);
    DrawLogicMenuWindowBorder(&sLogicMenuWindowTemplates[winId], MAIN_MENU_BORDER_TILE);
    CopyWindowToVram(WIN_LOGIC_MSG, COPYWIN_FULL);
}
...
static void ClearLogicMenuWinMsg(u8 winId)
{
    DrawDialogFrameWithCustomTile(winId, TRUE, BIRCH_DLG_BASE_TILE_NUM);
    DrawLogicMenuWindowBorder(&sLogicMenuWindowTemplates[winId], MAIN_MENU_BORDER_TILE);
    CopyWindowToVram(WIN_LOGIC_MSG, COPYWIN_GFX);
}
...
static void PrintLogicMenuHints()
{
    ...
    s16 x = GetStringCenterAlignXOffset(FONT_SMALL_NARROW, text, GetWindowAttribute(WIN_LOGIC_HINTS, WINDOW_WIDTH) * 8);
